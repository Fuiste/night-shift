...
pub fn execute(
  ...
  let target_doc_path = shared.resolve_doc_path(repo_root, doc_path)
  use notes_source <- result.try(shared.resolve_notes_source(
    repo_root,
    notes_value,
  ))
  ...
  use _ <- result.try(shared.write_string(target_doc_path, document))
  use #(seeded_run, replanning) <- result.try(shared.prepare_planning_run(
    repo_root,
    target_doc_path,
    planning_agent,
    execution_agent,
  ...
  Ok(workflow.PlanResult(
    ...
    warnings: [],
    next_action: shared.next_action_for_run(planned_run),
  ))
}
