...
pub fn usage() -> String {
  ...
  <> "\n"
  <> "Commands:\n"
  <> "  start --brief <path> [--harness <codex|cursor>] [--max-workers <n>]\n"
...
pub fn parse(args: List(String)) -> Result(types.Command, String) {
  case args {
    ...
    ["resume", ..rest] -> parse_run_lookup(rest, types.Resume)
...
fn parse_start(args: List(String)) -> Result(types.Command, String) {
  parse_start_flags(args, Error(Nil), Error(Nil), Error(Nil))
}
